...
const nutriScoreCalculator = async () => {
  try {
    ...
    for (const productId in products) {
      ...
      console.log(energy)
      const sugar = extractNumericValue(product.nutrisi.karbohidrat.gula) * factor;
      console.log(sugar)
      ...
      console.log(protein)
      const fiber = extractNumericValue(product.nutrisi.karbohidrat.serat || "0g") * factor;
      console.log(fiber)
      const type = product.type;
      const sub_type = product.sub_type;

      // NutriScore calculation
      const nutriScoreValue = calculateNutriScore(energy, sugar, satFat, salt, protein, fiber);

      // Determine NutriScore letter
      const nutriScoreLetter = determineNutriScoreLetter(nutriScoreValue, type, sub_type);
      ...
      await admin.database().ref('products').child(productId).update({ nutriscore });
    ...
    const NPoints = {
      energy: getNPoints(energy, [120, 240, 360, 480, 600, 720, 840, 960, 1080, 1200]),
    ...
    const totalNPoints = NPoints.energy + NPoints.sugars + NPoints.satFat + NPoints.salt;
...
const determineNutriScoreLetter = (score, type, sub_type) => {
    if (type === 'beverages') {
      ...
      if (score <= 9) return 'D';
      return 'E';
    } else if (type === 'vegetable fats' || type === 'animal fats' || type === 'seeds' || type === 'nuts') {
      if (score <= -6) return 'A';
      if (score <= 2) return 'B';
